...
class UserController extends \App\Controllers\BaseController
{
    ...
    public function postReNewPassword(Request $request, Response $response)
    {
        ...
        $token['token'] = $request->getQueryParam('token');

        try {
            $reNewPassword = $this->testing->request('POST', $this->router->pathFor('api.user.put.renew.password'), ['query' => $token,'json' => $body]);
            ...
            return $this->response->withRedirect($this->router->pathFor('web.user.renew.password')."?token=$token[token]");
        }
    }
